/// Character set translation for VT100/xterm compatibility.
///
/// Terminals use "charset designation" escape sequences to switch between
/// character sets.  The most common are:
///   - US ASCII (ESC ( B) — identity mapping
///   - DEC Special Graphics (ESC ( 0) — box-drawing characters
///
/// The active charset is selected via SI (0x0F) / SO (0x0E) which switch
/// between G0 and G1.  Each Gn slot holds a charset designation ('B', '0', etc.).
library;

/// DEC Special Graphics character set.
/// Maps ASCII 0x5f–0x7e to Unicode box-drawing and symbol characters.
...
/// Charset designation IDs (the character after ESC ( or ESC ) ).
const _charsetB = 0x42; // 'B' — US ASCII
const _charset0 = 0x30; // '0' — DEC Special Graphics

/// Translate [codepoint] through the given [charsetId].
///
/// [charsetId] is the raw character from the designation sequence
/// (e.g., 0x42 for 'B', 0x30 for '0').  A value of 0 means no
/// charset has been explicitly designated — defaults to ASCII.
int translateCharset(int charsetId, int codepoint) {
  if (charsetId == _charset0) {
    return decSpecGraphics[codepoint] ?? codepoint;
  }
  // ASCII ('B') or unset (0) — identity
  return codepoint;
}
