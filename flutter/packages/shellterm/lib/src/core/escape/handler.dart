...
abstract class EscapeHandler {
  ...
  /// Called when a DCS (Device Control String) sequence begins.
  /// [params] are the numeric parameters, [intermediates] any collected bytes.
  void dcsHook(List<int> params, List<int> intermediates);

  /// Called for each data byte during a DCS passthrough.
  void dcsPut(int byte);

  /// Called when a DCS sequence terminates.
  void dcsUnhook();
}
