...
import 'package:shellterm/src/core/buffer/range.dart';
import 'package:shellterm/src/core/buffer/segment.dart';
...
import 'package:shellterm/src/ui/cursor_type.dart';
import 'package:shellterm/src/ui/painter.dart';
import 'package:shellterm/src/ui/selection_mode.dart';
import 'package:shellterm/src/ui/terminal_size.dart';
import 'package:shellterm/src/ui/terminal_text_style.dart';
import 'package:shellterm/src/ui/terminal_theme.dart';

typedef EditableRectCallback = void Function(Rect rect, Rect caretRect);

/// Cached rendered image for a single terminal line.
class _LineCache {
...
class RenderTerminal extends RenderBox with RelayoutWhenSystemFontsChangeMixin {
  ...
  /// Per-line picture cache keyed by [BufferLine] object identity.
  /// Values are invalidated when [BufferLine.generation] changes.
  /// [LinkedHashMap] preserves insertion order for LRU eviction.
  final _lineCache = LinkedHashMap<BufferLine, _LineCache>();
  static const _lineCacheMaxSize = 2048;

  /// Clears the line picture cache. Must be called when visual output changes
  /// for reasons other than buffer mutations (e.g. theme/font change).
  void _clearLineCache() {
    for (final c in _lineCache.values) {
      c.picture.dispose();
    }
    _lineCache.clear();
  }
  ...
  void performLayout() {
    ...
    // Save _stickToBottom BEFORE _updateScrollOffset(), because
    // applyContentDimensions() triggers _onScroll() which re-evaluates
    // _stickToBottom using the OLD scroll position against the NEW
    // maxScrollExtent — incorrectly setting it to false when new content
    // arrives while already at the bottom.
    final wasStickToBottom = _stickToBottom;
  ...
  void _paint(PaintingContext context, Offset offset) {
    ...
    for (var i = effectFirstLine; i <= effectLastLine; i++) {
      ...
      final lineOffset = offset.translate(0, lineY);

      final cached = _lineCache[line];
      if (cached != null && cached.generation == line.generation) {
        // Fast path: draw cached picture.
        canvas.save();
        canvas.translate(lineOffset.dx, lineOffset.dy);
        canvas.drawPicture(cached.picture);
        canvas.restore();
      } else {
        // Slow path: record a new picture for this line.
        if (cached != null) cached.picture.dispose();
        final recorder = PictureRecorder();
        final lineCanvas = Canvas(recorder,
            Rect.fromLTWH(0, 0, lineWidth, charHeight));
        _painter.paintLine(lineCanvas, Offset.zero, line);
        final picture = recorder.endRecording();

        // Evict oldest entry if cache is full.
        if (_lineCache.length >= _lineCacheMaxSize) {
          final oldest = _lineCache.keys.first;
          _lineCache.remove(oldest)?.picture.dispose();
        }
  ...
  void _paintSelection(
    ...
    for (final segment in selection.toSegments()) {
      ...
      _paintSegment(canvas, paintOffset, segment, _painter.theme.selection);
    }
  }
  ...
  void _paintHighlights(
    ...
    for (var highlight in _controller.highlights) {
      ...
      for (var segment in range.toSegments()) {
        ...
        _paintSegment(canvas, paintOffset, segment, highlight.color);
      }
    }
  }
  ...
  void _paintSegment(
      Canvas canvas, Offset paintOffset, BufferSegment segment, Color color) {
    final start = segment.start ?? 0;
    final end = segment.end ?? _terminal.viewWidth;

    final startOffset = paintOffset +
        Offset(
