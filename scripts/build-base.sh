#!/bin/bash
# Build or pull the agentshell-flutter-base image.
# Tries GHCR first (fast), falls back to building locally.
#
# Usage:
#   ./scripts/build-base.sh          # auto: try GHCR, build if needed
#   ./scripts/build-base.sh --pull   # force pull from GHCR only
#   ./scripts/build-base.sh --build  # force local build only
...
GHCR_IMAGE="ghcr.io/clebermasters/agentshell-flutter-base:latest"
LOCAL_TAG="agentshell-flutter-base:latest"
DOCKERFILE="$PROJECT_ROOT/docker/flutter-base/Dockerfile"
...
try_build() {
    echo "Building base image locally (this takes ~15-20 min)..."
